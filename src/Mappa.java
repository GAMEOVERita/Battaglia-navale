...
public class Mappa {
    ...
    public void sceltaPosizione(char direzione, int DIM, int dimNave){
        ...
        inserisciNave(dimNave, direzione, posX, posY); // REFERENCE NON VISIBILE (è private all'interno di Mappa.java e va mpdificata)
    }
    ...
    private boolean isFree(int posX, int posY, int dimNave, char direzione, int DIM) {
        ...
        if (direzione == 'v'){ //se verticale
            ...
            for (int i = 0; i < dimNave; i++){
                ...
                if (mappa[posY-1+i][posX-1].getContenuto() == '+'){  //UGUALE A SOTTO
            ...
            // x mobile y fissa
            for (int i = 0; i < dimNave; i++){
                if (mappa[posY-1][posX-1+i].getContenuto() == '+'){  // DA SISTEMARE
                    System.out.println("LA NAVE INTERSECA CON UN'ALTRA GIA' ESISTENTE");
                    return false;
                }
            }
